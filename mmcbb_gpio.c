...
#include <wiringPi.h>						/* Include device specific declaration file here */

//#define		INIT_PORT()	init_port()			/* Initialize MMC control port (CS=H, CLK=L, DI=H, DO=in) */
#define 	DLY_US(n)	delay(n)	/* Delay n microseconds */

#define		CS_H()		digitalWrite(CE,HIGH)	/* Set MMC CS "high" */
#define 	CS_L()		digitalWrite(CE,LOW)	/* Set MMC CS "low" */
#define 	CK_H()		digitalWrite(CLK,HIGH)	/* Set MMC SCLK "high" */
#define		CK_L()		digitalWrite(CLK,LOW)	/* Set MMC SCLK "low" */
#define 	DI_H()		digitalWrite(MOSI,HIGH)	/* Set MMC DI "high" */
#define 	DI_L()		digitalWrite(MOSI,LOW)	/* Set MMC DI "low" */
#define 	DO			digitalRead(MISO)		/* Test for MMC DO ('H':true, 'L':false) */
