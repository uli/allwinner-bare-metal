...
void h264enc_free(h264enc *c)
{
	...
	// Don't free luma buffer, we get it from Engine BASIC.
...
h264enc *h264enc_new(const struct h264enc_params *p)
{
	...
	// Luma and chroma input buffers provided by Engine BASIC.
#if 0
...
int h264enc_encode_picture(h264enc *c)
{
	...
	// Luma buffer cache cleaned by Engine BASIC.
	...
	/* set input buffer */
	// Input buffers pointers provided by Engine BASIC are
	// identity-mapped address space, no need for conversion.
	//writel(ve_virt2phys(c->luma_buffer), c->regs + VE_ISP_INPUT_LUMA);
	//writel(ve_virt2phys(c->chroma_buffer), c->regs + VE_ISP_INPUT_CHROMA);
	writel((uint32_t)c->luma_buffer, c->regs + VE_ISP_INPUT_LUMA);
	writel((uint32_t)c->chroma_buffer, c->regs + VE_ISP_INPUT_CHROMA);
