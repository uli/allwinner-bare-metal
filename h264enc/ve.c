...
int ve_open(void)
{
	...
	// "reset sub-engines"; see https://linux-sunxi.org/VE_Register_guide#MACC_VE_RESET
	// Without this the encoder often does not restart after it has been used once.
	writel(0x101, ve.regs + VE_RESET);
	writel(0x100, ve.regs + VE_RESET);

	ioctl(ve.fd, IOCTL_ENGINE_REQ, 0);
	ioctl(ve.fd, IOCTL_ENABLE_VE, 0);
	ioctl(ve.fd, IOCTL_SET_VE_FREQ, 700);
